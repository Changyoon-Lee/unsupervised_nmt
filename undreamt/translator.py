...
class Translator:
    def __init__(self, encoder_embeddings, decoder_embeddings, generator, src_dictionary, trg_dictionary, encoder,
                 decoder, denoising=True, device=torch.device('cpu')):
        ...
        self.device = device
        weight = torch.ones(generator.output_classes(), device=device)
        weight[data.PAD] = 0
    ...
    def encode(self, sentences, train=False):
        ...
        with torch.no_grad():
            varids = torch.cuda.LongTensor(ids)
    ...
    def score(self, src, trg, train=False):
        ...
        # Decode
        initial_output = self.decoder.initial_output(len(src)).to(self.device)
        input_ids, lengths = self.trg_dictionary.sentences2ids(trg, eos=False, sos=True)
        input_ids_var = torch.LongTensor(input_ids).to(self.device)
        input_ids_var.requires_grad_(False)
        ...
        output_ids_var = torch.LongTensor(output_ids).to(self.device)
        output_ids_var.requires_grad_(False)
