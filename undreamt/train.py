...
def main_train():
    ...
    corpora_group.add_argument('--src', default='data/train_en_10k.txt', help='the source language monolingual corpus')
    corpora_group.add_argument('--trg', default='data/train_es_10k.txt', help='the target language monolingual corpus')
    ...
    embedding_group.add_argument('--src_embeddings', default='data/mapped_en.txt', help='the source language word embeddings')
    embedding_group.add_argument('--trg_embeddings', default='data/mapped_es.txt', help='the target language word embeddings')
    ...
    saving_group.add_argument('--save', default='enes', metavar='PREFIX', help='save models with the given prefix')
    saving_group.add_argument('--save_interval', type=int, default=100000, help='save intermediate models at this interval')
    ...
    if args.learn_encoder_embeddings:
        ...
        trg_encoder_embeddings = data.random_embeddings(trg_dictionary.size(), embedding_size).to(device)
    ...
    if args.fixed_generator:
        src_embedding_generator = EmbeddingGenerator(hidden_size=args.hidden, embedding_size=embedding_size).to(device)
        trg_embedding_generator = EmbeddingGenerator(hidden_size=args.hidden, embedding_size=embedding_size).to(device)
        ...
        src_generator = WrappedEmbeddingGenerator(src_embedding_generator, src_embeddings).to(device)
        trg_generator = WrappedEmbeddingGenerator(trg_embedding_generator, trg_embeddings).to(device)
    else:
        src_generator = LinearGenerator(args.hidden, src_dictionary.size()).to(device)
        trg_generator = LinearGenerator(args.hidden, trg_dictionary.size()).to(device)
    ...
    encoder = RNNEncoder(embedding_size=embedding_size, hidden_size=args.hidden,
                                bidirectional=not args.disable_bidirectional, layers=args.layers, dropout=args.dropout).to(device)
    add_optimizer(encoder, (src2src_optimizers, trg2trg_optimizers, src2trg_optimizers, trg2src_optimizers))

    # Build decoders
    src_decoder = RNNAttentionDecoder(embedding_size=embedding_size, hidden_size=args.hidden, layers=args.layers, dropout=args.dropout).to(device)
    trg_decoder = RNNAttentionDecoder(embedding_size=embedding_size, hidden_size=args.hidden, layers=args.layers, dropout=args.dropout).to(device)
